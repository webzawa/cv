...
##### オープンソースのライブラリの改修
使用していたオープンソースのライブラリで細かい挙動を制御できなかったため、直接ライブラリを改修しました。
本アプリでは認証機能にdeviseというRubyのライブラリを使用していましたが、
ある時ユーザの新規登録時に他ユーザの情報を紐付ける実装が必要となりました。
既存のライブラリの機能では思うように条件分岐などの実装ができないため、ライブラリをラップしたコードを作成することにより無事実装できました。
ライブラリのソースコードを理解するのに多少苦慮しましたが前職のSIerで他者の書いたソースコードを読み、デバッグしていた経験が活かせました。
以上のことからオープンソースのコードを読み、改修するスキルが身に付きました。
